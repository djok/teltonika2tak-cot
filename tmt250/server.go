...
func NewServer(ctx context.Context, wg *sync.WaitGroup, host string, port int, allowedIMEIs []string, callback PacketArrivedCallback) *Server {
	server := &Server{
		wg:                   wg,
		host:                 host,
		port:                 port,
...
func (s *Server) sendBytes(listen *net.UDPConn, data []byte, remote *net.UDPAddr) error {
	...
	_, err := listen.WriteToUDP(data, remote)
	if err != nil {
		return err
	}
